/**
 * Background Orchestrator — Fully autonomous browser automation.
 * Flow: Extract page FIRST → Plan → Act → Retry on failure until done.
 * Trust mode: zero manual intervention.
 */
...
let sessionState = { paused: false, stopped: false, running: false };
...
chrome.tabs.onUpdated.addListener(async (tabId, changeInfo, tab) => {
  ...
  if (!domain) return;
  const { [VISITED_KEY]: visited = [], uwa_trust_mode: trustMode, uwa_auto_goal: autoGoal } = await chrome.storage.local.get([VISITED_KEY, 'uwa_trust_mode', 'uwa_auto_goal']);
  if (!visited.includes(domain)) {
    ...
    setTimeout(() => chrome.runtime.sendMessage({ type: 'FIRST_VISIT', domain, url: tab.url }).catch(() => {}), 500);
  }
...
async function getDOMFromTab(tabId, waitForJs = true) {
  try {
    const response = await chrome.tabs.sendMessage(tabId, { type: 'GET_DOM_SNAPSHOT', waitForJs });
...
async function sendActionToTab(tabId, action) {
  const response = await chrome.tabs.sendMessage(tabId, { type: 'EXECUTE_ACTION', action });
...
async function startSession(userGoal, sendResponse) {
  ...
  sessionState = { paused: false, stopped: false, running: true };
  ...
  const opts = await chrome.storage.local.get(['llm_api_key', 'llm_api_url', 'llm_model', 'uwa_trust_mode', 'uwa_auto_fill_forms', 'uwa_user_profile']);
  ...
  const sendToContent = async (action) => {
    ...
    if (t === 'NAVIGATE' && action.url) {
      ...
      const waitMs = /chat\.openai\.com|chatgpt\.com/i.test(action.url) ? 6000 : 3500;
  ...
  const checkPermissionAutoApprove = async (perm) => {
    ...
    const check = await checkPermission(perm, { url: state.currentUrl });
    if (check.allowed === true) return true;
    if (check.allowed === false) return false;
    if (check.requiresApproval) {
      sessionState.pendingApproval = {};
      const prom = new Promise((res, rej) => {
        sessionState.pendingApproval.resolve = () => grantPermission(perm, { url: state.currentUrl }, 'once').then(() => res(true));
  ...
  try {
    ...
    for (let navLoop = 0; navLoop < MAX_NAV_LOOPS && !state.stopped; navLoop++) {
      ...
      broadcastLog('reader', 'Extracting full page content...', {});
      ...
      broadcastLog('decision_final', 'Planning actions from page content...', {});
      ...
      if (typeActions.length > 0) {
        if (trustMode || autoFillForms) {
          ...
          broadcastLog('agent', 'form_choices', { actions: typeActions, forms: state.pageData?.forms || [] });
          const choice = await new Promise((res) => { sessionState.formChoiceResolve = res; });
      ...
      for (let i = 0; i < state.actions.length && !state.stopped; i++) {
        ...
        while (tries < MAX_RETRIES_PER_ACTION && !state.stopped) {
          try {
            broadcastLog('executor', 'Executing', { type: action.type, selector: action.selector });
            let result;
            if ((action.type || '').toUpperCase() === 'NAVIGATE' && action.url) {
